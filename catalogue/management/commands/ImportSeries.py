...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.imp_series("CSV/Series.csv",options["DEBUG"]) #calls the function that imports the CSV at the file path into the Bible_Book data table. It also passes the result of options["DEBUG"] which checks if the debug command has been called.

    def imp_series(self,filepath,Debug):
        if Debug:
            print("The command ran and:") # Debug Text
        Series.objects.all().delete() # Clears all the existing data before reimporting, a useful but dangerous commands.
