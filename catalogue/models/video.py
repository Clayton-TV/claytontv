...
class Video(models.Model):
    ...
    bible_book = models.ManyToManyField(
        Bible_Book, blank=True, help_text="The bible books covered in the video."
    )
    demographic = models.ManyToManyField(
        "Demographic", blank=True, help_text="The video's demographic."
    )
    description = models.TextField(
        max_length=5000, help_text="Enter a brief description of the video <5000 chars."
    )
    url = models.URLField(unique=True, help_text="A link to where the video is hosted.")
    ministry = models.ManyToManyField(
        "Ministry", blank=True, help_text="The ministries associated with the video."
    )
    ...
    speaker = models.ManyToManyField(
        "Speaker", blank=True, help_text="The speakers/artist in the video."
    )
    ...
    date_recorded = models.DateField(
        null=True, blank=True, help_text="The date the video was recorded."
    )
    date_created = models.DateField(help_text="The date a video is uploaded.")
    date_modified = models.DateField(
        null=True, blank=True, help_text=" The last time the video data was edited."
    )
