...
@pytest.mark.parametrize("requested", ["0", "-3", "99999", "not-a-page"])
def test_browse_clamps_impossible_pages_instead_of_500ing(client, requested):
    # #329: /browse takes repeatable speaker/book/audience params, so its URL
    # space is combinatorial and crawlable — and unlike /search it isn't
    # disallowed in robots.txt.
    VideoFactory()

    response = client.get("/browse/", {"page": requested})
    props = inertia_page(response)["props"]

    assert response.status_code == 200
    assert len(props["videos"]) == 1
    assert props["page"] == 1
...
def test_browse_reports_the_effective_page(client):
    for _ in range(30):
        VideoFactory()

    props = browse(client, page=2)["props"]
...
@pytest.mark.parametrize(
    ("requested", "served"),
    [("0", 1), ("-3", 1), ("not-a-page", 1), ("99999", 2)],  # below 1 is page 1; past the end is the last page
)
def test_browse_serves_the_nearest_real_page(client, requested, served):
    # With more than one page, "clamped" has to mean the RIGHT end: page 0 is the
    # start of the list, not the end of it (get_page alone reads it as the end).
    for _ in range(30):
        VideoFactory()

    props = browse(client, page=requested)["props"]
